    ...
    implements OnGatewayConnection, OnGatewayDisconnect
{
    ...
    async handleUpdateComment(
        @MessageBody() { _id, content, idBlog, idCourse }: UpdateCommentDto,
    ) {
        try {
    ...
    async handleDeleteComment(client: Socket, commentId: string) {
        try {
            ...
            const comments = await this.commentModel
                ...
                .exec();
            const room = idBlog || idCourse;
            const email = commentId[3];
            const mess = commentId[4];
            if (email && mess) {
                const mailOptions = {
                    from: '<hieu@78544@gmail.com>',
                    to: email,
                    subject: 'FSTUDY báo cáo bình luận',
                    html: `
            <p><strong>Báo cáo: </strong>${mess}</p>
              `,
                };
