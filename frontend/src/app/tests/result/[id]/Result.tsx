...
function Result({ id }: { id: string }) {
    ...
    useEffect(() => {
        const getResultExam = async () => {
            try {
                const response = await ExamService.getResultByGroupQuestions(id);
            ...
            <div className="p-10 shadow-md m-10 h-auto">
                ...
                <Row gutter={[16, 16]}>
                    <Col xs={24} sm={24} md={16} lg={18} xl={19}>
                        ...
                        <h2 className="font-bold text-xl">Đáp án chi tiết</h2>
                        <Row gutter={[16, 16]}>
                            <Col xs={24} sm={12}>
                                {resultDetail?.examSessions.slice(0, Math.ceil(resultDetail.examSessions.length / 2)).map((session:any, sessionIndex:any) => (
                                    <div key={session.sessionId}>
                                        <h2 className='font-semibold text-lg'>{`Recording ${sessionIndex + 1}`}</h2>
                                        {renderAnswers(session.correctAnswers, "correct", showModal)}
                                        {renderAnswers(session.incorrectAnswers, "incorrect", showModal)}
                                        {renderAnswers(session.skippedAnswers, "skipped", showModal)}
                                    </div>
                                ))}
                            </Col>
                            <Col xs={24} sm={12}>
                                {resultDetail?.examSessions.slice(Math.ceil(resultDetail.examSessions.length / 2)).map((session:any, sessionIndex:any) => (
                                    <div key={session.sessionId}>
                                        <h2 className='font-semibold text-lg'>{`Recording ${sessionIndex + 1 + Math.ceil(resultDetail.examSessions.length / 2)}`}</h2>
                                        {renderAnswers(session.correctAnswers, "correct", showModal)}
                                        {renderAnswers(session.incorrectAnswers, "incorrect", showModal)}
                                        {renderAnswers(session.skippedAnswers, "skipped", showModal)}
                                    </div>
